...
export default {
  ...
  computed: {
    ...
    initialList () {
      let initialList = []
      let OfficialAccounts = this.$store.state.OfficialAccounts
      let max = OfficialAccounts.length
      for (let i = 0; i < max; i++) {
    ...
    OfficialAccountsList () {
      let OfficialAccountsList = {}
      let OfficialAccounts = this.$store.state.OfficialAccounts
      let max = OfficialAccounts.length
      for (let i = 0; i < this.initialList.length; i++) {
        let protoTypeName = this.initialList[i]
        OfficialAccountsList[protoTypeName] = []
        for (let j = 0; j < max; j++) {
          if (OfficialAccounts[j].initial === protoTypeName) {
            OfficialAccountsList[protoTypeName].pus(OfficialAccounts[j])
          }
        }
      }
